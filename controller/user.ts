...
import { bodyToJSON, check_username, check_new_username, check_password, compare_password, add_session, get_username } from "./common.ts"
 ...
 */
 export const signup = async (req: SystemRequest, res: SystemResponse) => {
    ...
    // ユーザー名が正しいかどうかをチェックする
    if(!check_username(user_name)) {
        res.setText("Invalid username.", 403);
        return;
    }

    // パスワードが正しいかどうかをチェックする
    if(!check_password(password)) {
        res.setText("Invalid password.", 403);
        return;
    }

    // ユーザー名が使用可能かどうかをチェックする
    if(!await check_new_username(user_name)) {
        res.setText("Username already exists.", 403);
        return;
    }
